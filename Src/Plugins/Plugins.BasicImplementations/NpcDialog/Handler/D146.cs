...
{
    public class D146 : INpcDialogAsyncHandler
    {
        public long HandledId => 146;
        ...
        public async Task Execute(ClientSession Session, NpcDialogEvent packet)
        {
           ...
           {
               if (Session.Character.Inventory.CountItem(2522) < 1)
               {
                   Session.SendPacket(UserInterfaceHelper.GenerateMsg(Language.Instance.GetMessageFromKey("NOT_ENOUGH_INGREDIENTS"), 0));
                   return;
               }
               Session.Character.GiftAdd(2518, 5);
               Session.Character.Inventory.RemoveItemAmount(2522, 1);
           }
        }
    }
}