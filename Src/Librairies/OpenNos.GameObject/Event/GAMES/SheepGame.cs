...
namespace OpenNos.GameObject.Event.GAMES
{
    public static class SheepGame
    {
        ...
        public static void GenerateSheepGames()
        {
            ...
            ServerManager.Instance.Broadcast("qnaml 1 #guri^506 want to partecipate?");
        ...
        public class SheepThread
        {
            ...
            public void Run(MapInstance map)
            {
                ...
                foreach (ClientSession session in _map.Sessions)
                {
                    ...
                    session.SendPacket("say 1 -1 10 30 seconds to start");
                    Observable.Timer(TimeSpan.FromSeconds(30)).Subscribe(o =>
                    {
                        session.SendPacket("say 1 -1 10 10 seconds to start");
                    });
                    Observable.Timer(TimeSpan.FromSeconds(35)).Subscribe(o =>
                    {
                        session.SendPacket("say 1 -1 10 5 seconds to start");
                    });
                    Observable.Timer(TimeSpan.FromSeconds(36)).Subscribe(o =>
                    {
                        session.SendPacket("say 1 -1 10 4 seconds to start");
                    });
                    Observable.Timer(TimeSpan.FromSeconds(37)).Subscribe(o =>
                    {
                        session.SendPacket("say 1 -1 10 3 seconds to start");
                    });
                    Observable.Timer(TimeSpan.FromSeconds(38)).Subscribe(o =>
                    {
                        session.SendPacket("say 1 -1 10 2 seconds to start");
                    });
                    Observable.Timer(TimeSpan.FromSeconds(39)).Subscribe(o =>
                    {
                        session.SendPacket("say 1 -1 10 1 seconds to start");
            ...
            private void End(Character character)
            {
                ...
                character.GenerateFamilyXp(character.Level * 40);
                character.GetReputation(character.Level * 50);
                character.GetGold(character.Level * 2500);
                character.GiftAdd(1134, 2);
                character.SpAdditionPoint += character.Level * 10;
