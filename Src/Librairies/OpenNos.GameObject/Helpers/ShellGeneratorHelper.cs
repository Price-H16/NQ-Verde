...
namespace OpenNos.GameObject.Helpers
{
    public class ShellGeneratorHelper
    {
        ...
        public List<ShellEffectDTO> GenerateShell(byte shellType, int shellRarity, int shellLevel)
        {
            ...
            var shellOptions = new List<ShellEffectDTO>();
            var optionsAlreadyOn = new List<object>();

            if (shellType < 8)
                letterMultiplier = shellLevel <= 70 ? 1 :
                    shellLevel <= 80 ? 3 :
                    shellLevel <= 90 ? 5 : 1;
            else if (shellType < 10) letterMultiplier = shellLevel * 2 + 1;
            ...
            if (!ShellType.ContainsKey(w)) return shellOptions;

            for (var g = 3; g <= 23; g += 2)
            {
                if (ShellType[w][g] == null) continue;

                var possibleOptions = new List<object>();

                for (var i = 1; i <= 20; ++i)
                {
                    var index = (int?) ShellType[w][g];
                    if (index == null) continue;

                    var option = ShellOptionLevel[index.Value]?[i];
                    if (option == null) continue;

                    possibleOptions.Add(option);
                }

                foreach (var t in optionsAlreadyOn)
                    if (possibleOptions.Contains(t))
                        possibleOptions.Remove(t);

                if (possibleOptions.Count == 0) continue;

                var generatedOption = possibleOptions[_rand.Next(possibleOptions.Count)];

                var shellTyp = (int?) ShellType[w][g + 1];
                if (shellTyp.HasValue && shellTyp.Value != 1 && _rand.Next(2) != 0) continue;

                var optionValue = GenerateOptionValue((int) generatedOption, (int) ShellType[w][g], shellLevel);
                var optionType = (int) generatedOption;
                var optionLevel = (int) ShellType[w][g];
        ...
        public int PerfumeGoldAmountFromLevel(short level)
        {
            ...
            if (level < 50) return 8000;

            if (level < 60) return 10000;

            if (level < 70) return 12500;
        ...
        {
            var optionLevel = d % 4 == 0 ? 4 : d % 4;
            var minimum = (int?) ShellOptionType[randomOptionId][(optionLevel - 1) * 2];
            var maximum = (int?) ShellOptionType[randomOptionId][1 + (optionLevel - 1) * 2];

            if (!minimum.HasValue || !maximum.HasValue) return null;

            var m = _rand.Next(minimum.Value, maximum.Value + 1);
            var value = (int) Math.Round((double) m / 1000 * shellLevel);

            if (m != 0) return value == 0 ? 1 : value;
