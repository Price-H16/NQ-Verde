...
namespace OpenNos.Core.Networking.Communication.ScsServices.Client
{
    ...
    public class ScsServiceClient<T> : IScsServiceClient<T> where T : class
    {
        ...
        /// <returns></returns>
        public TServiceInterface GetServiceProxy<TServiceInterface>() => (TServiceInterface) new AutoConnectRemoteInvokeProxy<TServiceInterface, IScsClient>(
                _requestReplyMessenger, this).GetTransparentProxy();
        ...
        private void RequestReplyMessenger_MessageReceived(object sender, MessageEventArgs e)
        {
            ...
            try
            {
                ...
                var method = type.GetMethod(invokeMessage.MethodName) ?? type.GetInterfaces()
                                 .Select(t => t.GetMethod(invokeMessage.MethodName)).FirstOrDefault(m => m != null);
