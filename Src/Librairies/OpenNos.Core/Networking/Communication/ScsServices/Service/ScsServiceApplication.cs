    ...
    public class ScsServiceApplication : IScsServiceApplication, IDisposable
    {
        ...
        private sealed class ServiceObject
        {
            ...
            public ServiceObject(Type serviceInterfaceType, ScsService service)
            {
                ...
                foreach (var serviceInterface in serviceInterfaceType.GetInterfaces()
                    .Union(new[] {serviceInterfaceType}))
        ...
        public void AddService<TServiceInterface, TServiceClass>(TServiceClass service) where TServiceInterface : class
        ...
        private static void SendInvokeResponse(IMessenger client, IScsMessage requestMessage, object returnValue,
            ScsRemoteException exception, object[] parameters = null)
        {
            try
            {
        ...
        private void Client_MessageReceived(object sender, MessageEventArgs e)
        {
            ...
            try
            {
                ...
                catch (TargetInvocationException ex)
                {
                    ...
                    if (innerEx != null)
                        SendInvokeResponse(requestReplyMessenger, invokeMessage, null,
                            new ScsRemoteInvocationException(invokeMessage.ServiceClassName,
                                serviceObject.ServiceAttribute?.Version ?? "", invokeMessage.MethodName,
                ...
                catch (Exception ex)
                {
                    SendInvokeResponse(requestReplyMessenger, invokeMessage, null,
                        new ScsRemoteInvocationException(invokeMessage.ServiceClassName,
                            serviceObject.ServiceAttribute?.Version ?? "", invokeMessage.MethodName, ex.Message, ex));
                }
            }
            catch (Exception ex)
            {
