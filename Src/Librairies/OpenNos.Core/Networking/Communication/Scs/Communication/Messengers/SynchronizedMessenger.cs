        ...
        public IScsMessage ReceiveMessage(int timeout)
        {
            while (_running)
            {
                lock (_receivingMessageQueue)
                {
                    ...
                    if (_receivingMessageQueue.Count > 0) return _receivingMessageQueue.Dequeue();
                ...
                if (!signalled) throw new TimeoutException("Timeout occured. Can not received any message");
            }
